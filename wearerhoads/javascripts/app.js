	...
	$('#menuProjects').click(function() {
		if(!collectionTransition){
			...
			if(!$('.menuSub').hasClass('shown')){ 
				$('#btnProjects').animate({
				    height: origWidth+$('.menuSub').height()+15+'px'}, 350, function() {
...
//				  $('#switcher').animate({marginTop:origWidth+50},350);
				...
				$('.menuSub').animate({
				    ...
				    $('#btnProjects').animate({
					    height: origWidth-$('.menuSub').height()-19+'px'}, 350, function() {
					    // Animation complete.
					    collectionTransition = false;
					  });
					$('.menuSub').toggleClass('shown');
...
//	});
	...
	function randomFunctions(){
		...
		*/
		...
		$('a[href="'+window.location.protocol + "//" + window.location.hostname+window.location.pathname+'"]').addClass('active');
		$("#navigation li a:not(#bnProjects)").click(function(){
			$("#navigation li a").removeClass('active');
			$(this).delay(250).addClass('active');
		});
		...
		$(".meta .download").click(function(e) {
            ...
            window.open('/singlePdf.php?src=' + src,
            		'_newtab');
            e.preventDefault();
            return false;
		});
...
//		});
    ...
    var $container = $('#front.grid, #lightbox, #video');
	    ...
	    $('.redirTo').click(function(e) {
	    	$(this).removeClass('redirTo');
	    });
